 ...
 *
 * 总结：
 *      write(int b)  若写出的是数字或者英文都是OK的，但是若是中文（写出不全），则会出现乱码，不建议使用
...
public class FileOutputStreamDemo1 {
    ...
    public static void main(String[] args) {
        ...
        try {
            fos = new FileOutputStream(file,true);  //true  在文本上追加内容

            //3.写出文本内容至磁盘上  void write(int b) 一个一个字节写出
            //换行
            fos.write("\r\n".getBytes());
            ...
            if(fos!=null){
                try {
                    fos.close();
                    //fos.write('C');  //java.io.IOException: Stream Closed
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
